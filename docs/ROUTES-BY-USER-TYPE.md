...
|--------|--------------------------|--------|-------------|
| **المنصة الرئيسية (كل الشاشات)** | — | `admin@platform.sa` أو `admin@gateway.sa` | `password` |
| **B2C** (مخصصون) | — | `user@individual.sa` أو `b2c@individual.sa` | `password` |
| **B2B** (مخصصون) | `demo-company` | `admin@company.sa` أو `b2b@company.sa` | `password` |
...
المستخدمون التاليون عند تسجيل الدخول من **`/login`** يظهر لهم **كل** شاشات المنصة الرئيسية (لوحة التحكم، الشحنات، الطلبات، المتاجر، المحفظة، المستخدمين، الدعم، التتبع، الأدوار، الدعوات، الإشعارات، العناوين، الإعدادات، التقارير، المالية، التدقيق، التسعير، الإدارة، KYC، DG، المنظمات، الحاويات، الجمارك، السائقين، المطالبات، السفن، الجداول، الفروع، الشركات، أكواد HS، المخاطر، وتسجيل الخروج):

| البريد | كلمة المرور | صفحة الدخول |
|--------|-------------|--------------|
| **`admin@platform.sa`** | `password` | **`/login`** |
| **`admin@gateway.sa`** | `password` | **`/login`** |

بعد الدخول يُوجّه إلى `/` وتظهر في الشريط الجانبي قائمة Admin الكاملة؛ كل رابط يفتح الشاشة المقابلة من الجدول في قسم «المنصة الرئيسية». يُنشأ هذان المستخدمان من السيدر `DevPlatformAdminSeeder` عند تشغيل `php artisan db:seed`.
