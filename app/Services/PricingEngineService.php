...
class PricingEngineService
{
    ...
    private function mapBreakdownToShipmentQuotePricing(PricingBreakdown $breakdown, array $carrierOffer): array
    {
        ...
        return [
            ...
            'pricing_breakdown' => [
                ['step' => 'net_rate', 'value' => round((float) $breakdown->carrier_net_rate, 2), 'description' => 'سعر الناقل الصافي'],
                ['step' => 'markup', 'amount' => round((float) $breakdown->markup_amount, 2), 'description' => 'هامش الربح'],
                ['step' => 'service_fee', 'amount' => round((float) $breakdown->service_fee, 2), 'description' => 'رسوم الخدمة'],
                ['step' => 'final', 'net_rate' => round((float) $breakdown->net_rate, 2), 'retail_rate' => round((float) $breakdown->retail_rate, 2), 'description' => 'السعر النهائي',
