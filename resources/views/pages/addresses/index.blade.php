...
@section('content')
<x-page-header title="العناوين" :subtitle="$subtitle ?? null">
            ...
            @if(isset($c['actions']))
                <div class="card-actions">
                    @foreach($c['actions'] as $act)
                        @if(isset($act['method']))
                            <form action="{{ $act['url'] }}" method="POST" style="display:inline" onsubmit="return confirm('{{ $act['confirm'] ?? 'هل أنت متأكد؟' }}');">
                                @csrf
                                @method($act['method'])
                                <button type="submit" class="btn {{ $act['class'] ?? 'btn-s' }}">{{ $act['label'] }}</button>
                            </form>
                        @else
...
@if(isset($createRoute))
<x-modal id="create-addresses" title="إنشاء جديد">
    @if(isset($createForm))
        {!! $createForm !!}
    @endif
</x-modal>
